...
export class MainView extends LitElement {
    ...
    async _loadFromStorage() {
        try {
            const [prefs, creds] = await Promise.all([
                helpingHands.storage.getPreferences(),
    ...
    async _saveMode(mode) {
        ...
        this._tokenError = false;
        this._keyError = false;
        await helpingHands.storage.updatePreference('providerMode', mode);
        this.requestUpdate();
    }
    ...
    async _saveToken(val) {
        ...
        this._tokenError = false;
        try {
            const creds = await helpingHands.storage.getCredentials().catch(() => ({}));
            await helpingHands.storage.setCredentials({ ...creds, cloudToken: val });
        } catch (e) {}
        this.requestUpdate();
    }
    ...
    async _saveGeminiKey(val) {
        this._geminiKey = val;
        this._keyError = false;
        await helpingHands.storage.setApiKey(val);
        this.requestUpdate();
    }

    async _saveGroqKey(val) {
        this._groqKey = val;
        await helpingHands.storage.setGroqApiKey(val);
        this.requestUpdate();
    }
    ...
    async _saveOpenaiKey(val) {
        ...
        try {
            const creds = await helpingHands.storage.getCredentials().catch(() => ({}));
            await helpingHands.storage.setCredentials({ ...creds, openaiKey: val });
        } catch (e) {}
        this.requestUpdate();
    }
    ...
    async _saveOllamaModel(val) {
        ...
        await helpingHands.storage.updatePreference('ollamaModel', val);
        this.requestUpdate();
    }

    async _saveWhisperModel(val) {
        this._whisperModel = val;
        await helpingHands.storage.updatePreference('whisperModel', val);
        this.requestUpdate();
    }
